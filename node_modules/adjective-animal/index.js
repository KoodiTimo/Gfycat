...
function fileToArray(file)
{
  return file
    .split("\n")
    .map(animal => animal.trim().replace(" ",""))
    .filter(a => a!="");
}
...
{
  return adjectives[Math.round(Math.random()*adjectives.length)]+adjectives[Math.round(Math.random()*adjectives.length)]+animals[Math.round(Math.random()*animals.length)];
}
